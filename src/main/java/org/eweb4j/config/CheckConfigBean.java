...
public class CheckConfigBean {
	public static String checkEWeb4JConfigBean(ConfigBean cb) {
		...
		if (!"true".equalsIgnoreCase(cb.getReload())
			...
			sb.append("当前您填写的：( reload=").append(cb.getReload())
		...
		if (i18n != null){
			for (Locale l : i18n.getLocale()){
				...
				if (!flag){
					sb.append("当前您填写的：( language=").append(l.getLanguage()).append(", country=").append(l.getCountry())
						.append("  即：").append(l.getLanguage()).append("_").append(l.getCountry()).append(" )是错误的！它只能填写为：").append(locSB.toString());
					break;
				}
			}
		}
		...
		if (!"true".equalsIgnoreCase(ioc.getOpen()) && !"false".equalsIgnoreCase(ioc.getOpen()) && !"1".equals(ioc.getOpen()) && !"0".equals(ioc.getOpen())) {
			...
			for (LogConfigBean log : ioc.getLogs().getLog()) {
				...
				if (!"off".equalsIgnoreCase(level)
						...
						&& !"fatal".equals(level) && !"0".equals(level)
						&& !"1".equalsIgnoreCase(level)
						&& !"2".equalsIgnoreCase(level) && !"3".equals(level)
						&& !"4".equals(level) && !"5".equals(level)) {
		...
		if (!"true".equalsIgnoreCase(mvc.getOpen())
			...
			for (LogConfigBean log : mvc.getLogs().getLog()) {
				...
				if (!"off".equalsIgnoreCase(level)
						...
						&& !"2".equalsIgnoreCase(level) && !"3".equals(level)
						&& !"4".equals(level) && !"5".equals(level)) {
					sb.append("当前您填写的：( level=")
							.append(level)
							.append(" )是错误的！它只能填写为：off|info|debug|warn|error|fatal|0|1|2|3|4|5 中的一种 ;")
							.append("\n");
				}
